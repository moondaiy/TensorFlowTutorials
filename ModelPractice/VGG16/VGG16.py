...
class Vgg16:
    ...
    init function
    """
    def __init__(self, trainable = FLAGS.trainable, trainModel = tf.placeholder(tf.bool), inputData = tf.placeholder("float", [None, FLAGS.height, FLAGS.width, FLAGS.channel]),inputLabel=tf.placeholder("float", [None, 10]),dropOutRatio = 0.5, vgg16_npy_path=None):
        """
    ...
    def __build(self):
        ...
        """
        #其实可以全部改成私有变量
        self.conv1_1 = self.conv_layer(self.__input, "conv1_1")
        ...
        self.relu7 = tf.cond(self.__trainModel, lambda: tf.nn.dropout(self.relu7, self.dropout), lambda: self.relu7)
